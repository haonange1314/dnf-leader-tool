...
系统采用“用户 → 角色 → 权限”的 RBAC 模型，一个用户当前绑定一个角色，角色可以绑定多项
权限。权限按副本、人员、人员导入、排表、智能生成、发布、导出、分享、用户、角色和操作日志
划分，前端入口和后端接口使用同一权限码判断。

- Owner（系统所有者）：内置保护角色，始终启用并拥有全部权限。
- Editor（业务编辑者）：内置角色，默认拥有全部业务读写、生成、发布、导出和分享权限。
- Viewer（只读查看者）：内置角色，默认拥有业务读取和排表导出权限。
- 自定义角色：管理员可以按实际分工选择权限，例如只维护人员、只负责排表生成或只查看日志。

调整角色权限后撤销该角色账号的旧会话，使权限变化不会被旧页面长期缓存；停用仍有启用账号
的角色前必须先迁移这些账号。普通参团玩家仍不需要登录。
...
- 由团长手动录入伤害。
- 单位为“亿”。
- 业务输入必须为整数；数据库继续使用定点数以兼容历史快照和通用评分接口。
...
| 启用状态 | 是 | 玩家生命周期状态；停用后其角色不进入新排表候选池，历史排表不受影响 |
...
| 是否默认加入新排表 | 是 | 创建排表时决定是否初始勾选；关闭后仍可从候选池手动选择 |
| 备注 | 历史兼容 | 保留旧数据，不在人员页面录入或展示 |
| 启用状态 | 是 | 角色生命周期状态；停用后不进入新排表候选池，历史排表不受影响 |
...
- 人员页面默认是紧凑浏览模式，不提供搜索入口；Excel 全量同步是日常主要维护方式，新增、修改、停用、删除和拖拽等低频操作统一收纳在显式开启的“手动维护”模式中。
- 支持新增玩家并连续添加多个角色。
- 支持新增、编辑、停用和永久删除玩家及角色；已被排表引用的记录禁止永久删除并提示改用停用，避免破坏历史数据。
- 支持拖拽调整玩家顺序及每名玩家内部的角色顺序，刷新或重新登录后保持。
...
3. 校验列名、类型、必填字段和数值格式；默认情况下，“是否秘宝C”“固定红队奶”“是否群猎”空白按“否”处理，“是否参与团本”空白按“是”处理。四项空白默认值由服务端统一配置，下载模板说明和导入解析必须保持一致。
4. 按“玩家昵称”自动创建或匹配玩家。
5. 按“玩家昵称 + 职业”匹配已有角色；文件内或数据库中同玩家重复职业均拒绝写入。
6. 展示新增、更新、恢复玩家、忽略、将停用玩家、将停用角色、顺序调整和错误的变更预览；除汇总数量外必须列出具体玩家、职业、Excel 行号和变更字段，行级错误在页面内按 Excel 行号分页展示，错误文件仅作为辅助下载；空数据文件直接拒绝，避免误停用全部人员。
7. 团长确认后在单个数据库事务中按全量名单同步，并按 Excel 数据行首次出现的顺序排列玩家及其角色；重新出现在文件中的玩家和角色恢复启用，未出现在表格中的既有玩家和角色采用软停用，保持原相对顺序并排在导入项之后，历史排表不受影响。
8. 页面错误明细支持分页查看，并可按需下载错误文件。
9. 页面可把当前启用玩家和启用角色按持久化顺序导出为同格式 Excel，作为下一次全量维护底稿；停用记录不进入导出，避免回传时被意外恢复。
10. 页面保留最近导入批次，能够查看文件名、预览/已同步状态、汇总和当时持久化的具体变更；历史批次用于审计，不提供一键回滚。

再次导入同一玩家的同职业角色时默认更新已有数据，但必须经过变更预览确认。导入文件应始终包含本次同步后需要保留启用的完整人员名单，而不是仅包含增量记录。
导入使用“玩家昵称 + 职业”作为业务匹配键；修改玩家昵称应先使用页面修改功能，再导出最新人员表继续维护。
...
新建排表时，所有玩家和角色均处于启用状态的角色都会进入当前排表候选快照；角色档案中的“默认加入新排表”只决定初始是否勾选。团长可以临时勾选默认不加入的角色，之后在排表中增减参团角色也不反向修改角色档案。
...
2. 名称存在歧义、引用不存在、规则不受支持，或与当前副本、玩家可用波次、最多出场次数及角色允许队伍冲突时，禁止确认并给出具体问题。
3. 团长确认后形成当前排表的规则集；修改原文必须重新解析和确认。
4. 自动生成只读取已确认的结构化规则集，并在结果中说明满足、未满足和发生冲突的规则。
5. 每次生成保存实际使用的规则快照、模型标识和解析版本，确保历史结果可以复现和解释。
6. 参团人员、玩家可用波次、最多出场次数、波数或副本结构变化使解析上下文失效时，规则集标记为“待重新解析”，在重新确认前不参与生成；拖拽和锁定变化只触发确定性冲突检查，不重新调用模型。
...
- 位于编辑器队伍区上方，可收起以节省纵向空间，并作为将已分配角色拖回的明确落点。
- 支持按角色类型筛选，并按人员顺序、玩家或角色数值排序。
- 明确区分“容量不足”“玩家波次冲突”“角色类型不匹配”等未分配原因。

### 9.5 操作与生成反馈

- 顶部只保留预检查、自动排表和发布等主操作，复制、同步、导出和生命周期操作收进“更多”。
- 进入自动排表前自动执行一次预检查；错误阻止生成，警告进入生成确认页展示。
- 波次导航同时显示空白、部分安排和完整状态，预检查中的波次问题可以直接定位。
- 生成记录从服务端恢复，最近两次用于质量对比；人工微调后保留记录并标明其对应的历史修订。
- 编辑租约丢失或未持有时，具备权限的用户可以在当前页面刷新状态、获取或接管编辑权。
...
- 已归档：只读保存，不参与默认列表；可显式显示，并恢复为草稿继续处理。

排表删除分为两类：普通历史清理使用可恢复的归档；永久删除仅允许从未形成发布版本的草稿，
要求独立权限、当前编辑租约、最新修订号及准确输入排表名称。已经发布过的排表只能归档，
不得通过删除破坏历史版本、分享链接或审计追踪。
...
- 能设置 C 伤害、奶评分、秘宝 C、角色状态和新排表默认选择。
...
- 相同人员、规则、锁定、求解时限和方案种子应可复现同一套候选方案；团长可以递增方案种子
  探索其他可行编队。
- 连续生成时展示前后两次关键质量指标变化，比较遵循安排人数、完整波次/队伍、优先组成、
  核心覆盖、强度顺序和跨波差的既定优先级，不用低优先级改善掩盖结构退化。
...
- 冲突能够实时显示并定位。
- 发布后形成不可变版本。
- 能查看、复制、归档和恢复历史排表，默认列表可隐藏归档项。
- 从未发布的草稿可在独立权限及名称二次确认下永久删除，已发布过的排表不可删除。
...
- 用户管理、角色权限和操作日志使用独立入口。
- 能创建自定义角色并配置模块权限，后端按权限码拒绝越权请求。
- 能查看账号状态、最近登录和有效会话数，并撤销其他有效会话。
- 系统所有者角色不能停用或移除权限，最后一个启用 Owner 账号不能被降权或停用。
- 操作日志支持按结果、动作和关键字分页检索，并可查看结构化上下文。
...
1. 2C2奶 时奶强度采用求和、主奶评分还是其他展示方式。
2. 自动排表单次求解的时间上限。
3. 波数安全上限及超大排表的导出方式。
4. 正式服务器的监控、告警和日志保留周期。
5. 长图的视觉主题、尺寸和群聊中的主要查看设备。
