...
> 文档版本：v0.2  
> 状态：需求确认稿  
> 当前范围：国服 PC 端 DNF、副本管理和 12 人团本排表  
> 暂缓范围：军团本、实时多人协作
...
- 团长：维护人员、创建排表、自动生成、微调、发布和导出。
- 协作者：首期共享同一套数据，但同一张排表同时只允许一人编辑。
- 查看者：查看已发布排表和下载导出文件。
...
副本管理提供通用人数和队伍结构，但不代表首期实现军团本的人工互带工作流。军团本后续可以复用同一副本版本模型，再增加专用分组页面和业务规则。
...
- 支持按玩家、角色名、职业、类型、秘宝 C、默认参团和状态筛选。
- 支持批量修改“默认参团”和启用状态。
...
3. 按“玩家称呼”自动创建或匹配玩家。
4. 按“玩家称呼 + 角色名”匹配已有角色。
5. 展示新增、更新、忽略和错误的变更预览。
6. 团长确认后在单个数据库事务中写入。
7. 生成可下载的错误明细。

再次导入相同角色时默认更新已有数据，但必须经过变更预览确认。
...
1. 最大化被安排的参团角色数量。
2. 优先填满较早波次，将待补集中到最后几波。
3. 最大化满足完整队伍配置的数量，并按副本版本中的组成优先级选择；内置团本优先 3C1奶。
4. 最大化满足副本特殊角色规则的完整波次数；内置团本即红队核心秘宝 C。
5. 分别最小化完整波次间 C 总伤害差和奶总评分差。
6. 最大化满足副本强度顺序配置的数量；内置团本即 `红 ≥ 黄 ≥ 绿`。
