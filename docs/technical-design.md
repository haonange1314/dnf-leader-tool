...
排表编辑页采用主次操作分层：预检查、自动排表和发布常驻，元数据、偏好、同步、复制、导出及生命周期操作进入次级菜单。队伍、位置、角色标签和拖拽落点封装在独立 `ScheduleEditor` 组件中，页面容器只编排 API 状态与弹窗。未分配池本身注册为 dnd-kit droppable，支持队伍角色执行通用 `UNASSIGN_PARTICIPANT` 命令；波次导航状态只读取服务端返回的位置快照，不重新计算业务评分。自动排表入口先调用服务端校验接口，生成历史继续以 `generation_runs` 为唯一数据源，人工命令仅标记历史结果与当前 revision 的对应关系，不清除已确认规则或生成记录。
...
为了减少候选角色全波次可用时的大量等价解，完整度阶段可以把具有已知有限上界的靠前
填充和特殊角色数量作为搜索提示中的低级 tie-break。系数必须由目标上界计算并检查 64 位
范围；阶段结束后仍只固定当前正式目标，再按词法顺序独立求解后级目标，因此 tie-break
不能用更好的低级结果补偿完整度退化。

同一玩家拥有大量约束等价角色时，可在安排人数阶段先建立独立的聚合可行性模型：以
“玩家 + 角色类型 + 可用波次 + 允许队伍”为分组，保留玩家同波唯一、角色组容量、锁定、
队伍容量、合法组成和特殊角色数量等硬约束。聚合模型同样先最大化安排人数，再依次优化组成
和特殊角色；若通用玩家/位置上界受队伍组成限制而不可达，则返回当前已证明或已找到的可行
人数下界，而不是把全部预搜索时间耗在不可达等式上。展开后的角色分配作为主模型 Hint，
主模型仍保留全部候选角色并独立执行正式词法目标。

聚合 Hint 展开角色身份时，先依据副本版本的角色类型强度顺序把高分角色放入更强队伍；
同一强度档内再选择当前该角色类型累计分较低的波次，避免按波次正序映射让高分同时堆叠在
前几波。该贪心只影响可被主模型改写的起始 Hint，不创建约束，也不识别红黄绿固定队伍键。

进入每个正式阶段后，只有在前级目标已经固定的结果空间内，才用当前可行 Hint 的正式阶段值
防止限时搜索退化，并允许继续改善；完整度搜索中的靠前填充、特殊角色等低级 tie-break 不得
作为后续阶段的持久约束。聚合搜索使用固定随机种子和确定性交错的并行 worker，避免相同输入
在质量回归中因线程调度得到不同门槛结果。聚合结果只用于减少对称搜索，不能成为最终解、
裁剪候选池、越过前级目标或改变副本版本规则。
...
`balanceAcrossWaves` 的数组顺序也是指标的词法优先级。求解器逐项求解并固定当前结果，
不会把 C 与奶的差值相加成一个可互相补偿的混合目标。若某阶段在时限内无法改善既有
可行解，则固定进入该阶段前的目标值并把阶段标记为“限时可行”，后级目标仍不能造成
该值退化。强度顺序和跨波差等松弛目标会按已有编队重新计算实际惩罚上界，不能直接采用
前一阶段尚未优化的辅助变量值；后级阶段完成后，诊断值再按最终编队重新计算。
...
每次生成在 `diagnostics.objectiveStages` 中按执行顺序保存阶段代码、目标值、耗时和结果
状态：`OPTIMAL` 表示 CP-SAT 已证明最优，`TARGET_REACHED` 表示已达到服务端可证明的
理论界，`FEASIBLE` 表示在阶段时限内保留了可行值。前端只解释这些确定性结果，不自行
推导求解质量。只要任一适用阶段仍为 `FEASIBLE`，生成结果的总体求解状态也不得标记为
`OPTIMAL`。
...
- `TIMEOUT`：达到求解时限仍未找到可行结果，不得误报为硬约束冲突；API 返回当前参数和建议的时限、种子。
- `ERROR`：求解器内部异常；输入校验错误由 API 在进入求解前单独返回。
...
前端提供“换一个方案”入口，以递增种子重新打开生成确认；连续生成完成后对比前后关键质量
指标。对比仅用于解释和人工选择，不自动覆盖服务端词法目标，也不把限时可行误标为已证明最优。
...
另以 `make test-quality` 运行 14 个固定 12 波质量画像：标准 3C1奶、2C2奶回退、秘宝不足、
可用波次短缺、同玩家角色冲突、锁定秘宝、全波次灵活的集中空位、匿名 11/12 玩家多角色
画像、前后半程分段可用画像、强分差下固定主队奶与每玩家最多波数的组合画像，以及已确认
玩家波次硬规则、角色指定波次规则与最多波数的组合画像、多个指定角色与三玩家互斥共同
生效的复杂规则交集画像。测试同时
校验摘要指标、各波填充、未分配原因及
角色/玩家/锁定/秘宝不变量；详细门槛见
`docs/raid-quality-baseline.md`。
