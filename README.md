...
> 当前状态：阶段 5 公网化工程基线已完成。项目已具备完整业务闭环、多账号权限、单编辑会话、Caddy 自动 HTTPS、生产环境强校验、备份恢复演练，以及 Playwright 浏览器闭环和 1/12/30/50 波性能回归；当前工作以现有 12 人团本流程的稳定性、实际使用反馈、易用性优化和正式上线准备为主。DeepSeek 自然语言规则闭环已实现，默认关闭，配置服务端密钥后启用。
...
- **自然语言排表规则（首批已实现）**：使用 DeepSeek v4 解析“本次排表要求”，经结构化预览和团长确认后再由确定性编译器交给 OR-Tools；支持玩家波次限制/偏好、玩家波次互斥、角色指定波次/队伍和连续出场偏好，并在确认前检查玩家可用波次、最多出场次数和角色允许队伍，相关上下文变化后自动要求重新解析。长期规则仍归属于版本化副本配置。
...
- **人员管理**：默认采用紧凑浏览界面，以九列 Excel 全量同步为主要维护方式，少量调整使用页面表单；不录入角色名和备注，同一玩家职业唯一。支持新增、修改、停用、永久删除，以及玩家和角色两级拖拽排序并持久化。被排表引用的玩家或角色只能停用，未被引用的记录可永久删除。确认导入后按 Excel 行顺序排列人员和角色，文件中不存在的既有玩家或角色会安全停用并置于其后，重新出现在文件中的玩家和角色会恢复启用，同时保留历史排表及秘宝、固定红队奶与群猎标记。页面可导出当前启用人员作为下一次维护底稿，并查看导入历史及每批具体变更。
...
- **人工微调**：支持拖拽、交换、拖回未分配池、锁定、撤销恢复和局部重新生成；未分配角色可按类型、数值或玩家顺序快速整理，波次导航直接显示空白、部分和完整状态。
- **排表生命周期**：发布不可变排表快照，支持复制旧排表、归档和恢复归档；只有从未发布的草稿可由具备专门权限的账号输入名称后二次确认永久删除。
- **分享导出**：支持可过期、可撤销的只读链接，以及 PNG 长图、Excel 和纯文本；草稿导出强制显示水印。
- **账号与 RBAC**：用户管理、角色权限和操作日志使用独立页面；内置 Owner/Editor/Viewer 可兼容原有账号，也可新增自定义角色并按副本、人员、导入、排表、发布、导出、分享和系统管理能力授权。权限变更会撤销相关旧会话，写请求继续使用会话绑定 CSRF 令牌，登录失败按账号和来源限流。
- **单编辑会话**：进入排表时申请独立租约并自动续期；其他会话只读查看，租约过期后可主动刷新、获取或接管编辑权，写操作仍同时校验 revision。
...
flowchart LR
    ...
    API -. 显式解析请求 .-> DeepSeek["DeepSeek v4 API"]
...
- [12 人团本质量基线](docs/raid-quality-baseline.md)：典型人员短缺、秘宝、锁定、玩家冲突、匿名多角色和分段可用画像的量化门槛。
...
Owner 登录后可分别使用“用户管理”“角色与权限”和“操作日志”：创建或停用账号、重置密码、
撤销在线会话、建立自定义角色、配置权限矩阵，并分页检索登录及写操作。系统所有者角色保持
启用并固定拥有全部权限，避免误操作导致系统失去管理员。
...
Excel 导入的布尔列留空时，默认按“秘宝 C=否、固定红队奶=否、群猎=否、参与团本=是”处理。
可在 `.env` 中通过 `IMPORT_DEFAULT_TREASURE_DAMAGE`、
`IMPORT_DEFAULT_FIXED_LEAD_TEAM_BUFFER`、`IMPORT_DEFAULT_GROUP_HUNT` 和
`IMPORT_DEFAULT_RAID_PARTICIPANT` 调整；下载模板的填写说明与实际解析始终使用同一组配置，
重新构建并启动 API 后生效。
人员 Excel 按全量名单同步：预览会逐项显示新增、更新、恢复及将停用的玩家和角色，确认后文件外记录采用软停用，
不会破坏历史排表；空数据文件会被拒绝，避免误停用全部人员。行级校验错误会直接在人员页面分页展示，
无需先下载错误文件。C 伤害必须为整数，奶量可保留两位小数；需要修改玩家昵称时先在页面修改，
再导出当前人员表继续维护，避免全量导入把新昵称识别为新玩家。
...
- [x] 完成多账号、可配置 RBAC、用户/角色/操作日志独立管理、CSRF、登录限流和审计基线
...
- [x] 优化排表管理操作层级、波次导航、未分配池、玩家偏好、编辑权恢复和持久化生成记录展示
...
→ 多账号、单编辑会话锁与公网部署
→ 浏览器闭环、备份恢复和性能验收
→ DeepSeek 规则解释、团长确认与 OR-Tools 确定性执行
```
