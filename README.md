...
当前不再按新的产品阶段连续扩张功能，优先处理：

- 现有副本、人员、排表、发布和导出闭环的缺陷与易用性问题。
- 使用真实团长数据验证规则、求解结果、多人权限和单编辑会话体验。
- 按实际域名和服务器完成生产配置、证书签发、监控、日志与异地备份交接。
- 在出现真实性能瓶颈时再细化前端依赖拆包、长列表和超大排表体验。

军团本人工分组和互带规则仅保留为长期 TODO，目前不作为重点工作，也没有排期。其他长期 TODO 包括多人实时协作、玩家自助报名、动态评分公式和实战数据校正。
